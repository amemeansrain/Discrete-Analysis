...
\begin{itemize}
    \item \textbf{Суффиксный массив}: для исходного текста строится суффиксный массив методом удвоения. Для корректного построения к строке добавляется терминальный символ \texttt{'\textbackslash0'}, который лексикографически меньше остальных символов.

    \item \textbf{Массив LCP}: после построения суффиксного массива строится массив наибольших общих префиксов соседних суффиксов. Это позволяет избежать повторного сравнения одних и тех же символов при поиске образцов.

    \item \textbf{RMQ}: над массивом LCP строится Sparse Table для быстрых запросов минимума на отрезке. Это позволяет за \(O(1)\) получать LCP двух произвольных суффиксов, зная их позиции в суффиксном массиве.

    \item \textbf{Оптимизация поиска}: поиск границы диапазона суффиксов выполняется с использованием LCP-ускоренного бинарного поиска. За счёт хранения уже известного LCP с левой и правой границами уменьшается количество посимвольных сравнений.

    \item \textbf{Оптимизация вывода}: вывод накапливается в строковом буфере, после чего записывается крупными блоками. Это уменьшает накладные расходы на большое количество операций вывода.
\end{itemize}
